    ...
    return (
      <div className="my-2">
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
          <div className="bg-green-500 text-white text-xs font-semibold px-4 py-2">
            Suggested edit
          </div>
          <div className="px-4 py-3 space-y-1 text-sm">
            ...
            <div><span className="text-slate-400">New value:</span> <span className="font-bold text-green-600">RM {Number(content.value).toLocaleString()}</span></div>
          </div>
          <div className="px-4 pb-3 text-xs text-slate-500 italic">{content.summary}</div>
          <div className="px-4 pb-4 flex gap-2">
            <button
              onClick={onConfirm}
              className="flex-1 py-2 text-sm bg-green-500 text-white rounded-xl font-medium hover:bg-green-600 transition-colors"
            >
              Confirm
            </button>
            <button
